...
bool delete_node(Node*& root, int key) {
  ...
  // switch target with predecessor while recursively calling delete
  //}
 ...
 */
int numNodes( Node* root ) { //O(n)
	if(root == NULL) { //Empty Tree
		return 0;
	}
	return 1 + numNodes(root->left)+numNodes(root->right); //1 is the root
}

/**
 * Returns the number of leaves in the tree rooted at root.
 */
int numLeaves( Node* root ) { //O(n) going both ways on the recursion
	if(root == NULL) { //empty tree
		return 0;
	}
	else if ( (root->left == NULL) and (root->right == NULL) ) { // Just root
		return 1;
	}
 ...
 * # of edges on the longest path from the node to a leaf. A leaf node will have a height of 0.
 */
int height( Node* x ) { //O(n)
	if (x == NULL) {
		return -1;
	}
	else {
 ...
 * # of edges from the node to the tree's root node
 */
// int depth( Node* root, Node* x ) {
//   if (x == root) { //base case(s) => only root
//   	return 0;
//   }
//   if (root == NULL) { //base case(s) => empty tree
//   	return -1;
//   }
...
int depth( Node* root, Node* x, int d) { //O(lgn)
	if (x == root) { //base case(s) => only root
  	return 0;
  }
  if (root == NULL) { //base case(s) => empty tree
 ...
 * Traverse a tree rooted at rootNode in-order and use 'v' to visit each node.
 O(n)
 */
void in_order( Node* rootNode, int level, Visitor& v ) {//left, root, right
  if (rootNode == NULL) {
  	return;
  }
  else {
 ...
 * Traverse a tree rooted at rootNode pre-order and use 'v' to visit each node.
 O(n)
 */
void pre_order( Node* rootNode, int level, Visitor& v ) { //root, left, right
  if (rootNode == NULL) {
  	return;
  }
  else {
 ...
 * Traverse a tree rooted at rootNode post-order and use 'v' to visit each node.
	O(n)
 */
void post_order( Node* rootNode, int level, Visitor& v ) { //left, right, root -- bottom-up
	if (rootNode == NULL) {
		return;
	}
	else {
