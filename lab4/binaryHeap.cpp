...
//    cout << string(d, '*') + to_string(heap[node]) << endl; //e.g. * + to_string(heap[1])
//    d++;
//    
//    if (leftChild < size) {
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) { // O(size1+size2)
	int *temp = new int[size1+size2]; //array the size of both heaps combined
	int i = 0, j = 0, k = 0;
	while(k < size2 and j < size1){ 
		if(heap1[j] < heap2[k]){ //comparing the values of same position in respective heaps
			temp[i] = heap1[j];
			j++;
		}else{ // if value in heap2 is larger
			temp[i] = heap2[k];
			k++;
		}
		i++;
	}
	if (i < size1+size2){ //When you outgrow one of the heaps
		while(j < size1){
			temp[i]=heap1[j];
			j++;
			i++;
 	...
 	return temp; //return the final heap
}
