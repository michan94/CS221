...
int word_frequency_helper(string_bst::node_t * n, tree_key const & key) {
	if (n == NULL) { 
		return 0;
	}
	else if (n->data.word == key) {
	...
	else if (key < n->data.word) { //key is what youre looking for
		return word_frequency_helper(n->left, key);
	}
	else {
		return word_frequency_helper(n->right, key);
	}
}
